// ==UserScript==
// @name         craber 抓包器（通义千问）
// @namespace    tongyi-craber-sniffer
// @version      0.1.0
// @description  临时抓包脚本：拦截通义千问页面的 fetch/XHR，打印接口 URL/方法/请求体/响应/鉴权，用于分析数据结构。分析完即可卸载。
// @author       craber
...
(function () {
  ...
  function interesting(url) {
    ...
    // 只看通义自己的 api（tongyi.com / aliyun.com / dashscope 等）或相对路径，且命中关键词
    const isOwn = u.charAt(0) === '/' ||
      u.indexOf('qianwen.com') >= 0 || u.indexOf('tongyi.com') >= 0 ||
      u.indexOf('aliyun.com') >= 0 ||
      u.indexOf('dashscope') >= 0 || u.indexOf('alibaba') >= 0;
